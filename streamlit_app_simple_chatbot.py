...
def main():
    ...
    if len(st.session_state.messages) == 0:
        process_user_input("Donne-moi des exemples de questions")
...
def show_header_and_sidebar():
    ...
    st.title("Analyse des commandes à SummitSport")
    st.markdown(
        "Bienvenue sur Cortex Analyst! Pose-moi tes questions pour parler à tes données. "
    )
    ...
    with st.sidebar:
        ...
        if btn_container.button("Effacer l'historique", use_container_width=True):
            reset_session_state()
...
def process_user_input(prompt: str):
    ...
    with st.chat_message("analyst"):
        with st.spinner("En attente de la réponse de Cortex Analyst"):
            time.sleep(1)
...
def get_analyst_response(messages: List[Dict]) -> Tuple[Dict, Optional[str]]:
    ...
    request_body = {
        "messages": messages,
        "semantic_view": "SS_101.HARMONIZED.ORDERS_SV"
    }
